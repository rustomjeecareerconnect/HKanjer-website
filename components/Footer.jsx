        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-4 mt-2">
              <a href="#" target="_blank" rel="noopener noreferrer" className="text-text-dark-muted dark:text-text-muted hover:text-accent transition-colors" aria-label="LinkedIn">
                <FaLinkedin size={22} />
              </a>
              <a href="#" target="_blank" rel="noopener noreferrer" className="text-text-dark-muted dark:text-text-muted hover:text-accent transition-colors" aria-label="Google Scholar">
                <FaGraduationCap size={22} />
              </a>
              <a href="#" target="_blank" rel="noopener noreferrer" className="text-text-dark-muted dark:text-text-muted hover:text-accent transition-colors" aria-label="ResearchGate">
                <FaResearchgate size={22} />
              </a>
              <a href="#" target="_blank" rel="noopener noreferrer" className="text-text-dark-muted dark:text-text-muted hover:text-accent transition-colors" aria-label="Amazon Author Central">
                <FaAmazon size={22} />
              </a>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              {['Teaching', 'Consulting', 'Masterclass', 'Research', 'Media'].map((item) => (
                <li key={item}>
                  <Link href={`/${item.toLowerCase()}`} className="text-sm text-text-dark-muted dark:text-text-muted hover:text-accent animated-underline inline-block pb-1">
                    {item}
                  </Link>
                </li>
              ))}
